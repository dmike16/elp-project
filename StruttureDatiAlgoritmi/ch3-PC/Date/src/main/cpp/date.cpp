...
Chrono::Date::Date(int dd, Month mm, int yy)
{
	...
	if (dd == 0) dd = default_date.getDay();
...
inline int Chrono::Date::getDay() const
{
	return d;
}

inline Chrono::Date::Month Chrono::Date::getMonth() const
{
	return Month(m);
}

inline int Chrono::Date::getYear() const
{
	return y;
}
...
std::ostream& Chrono::operator<<(std::ostream& os, const Chrono::Date& date)
{
